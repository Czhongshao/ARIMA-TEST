...
def make_predictions_v2(model, scaler, data, raw_values, steps_ahead):
    predictions = []
    last_window = data[-1, :-1].reshape(1, 1, -1)
    
    last_actual_value = raw_values[-1]
...
"""
    ...
    '德国', '法国', '英国', '瑞士', '荷兰', '其他'
"""
need_to_predict = str(input("请输入你需要预测的国家："))
...
# 训练 LSTM 模型
lstm_model = fit_lstm(train_scaled, batch_size=1, epochs=500, neurons=4)
...
future_predictions = make_predictions_v2(lstm_model, scaler, test_scaled, raw_values, years_to_predicted)


# 绘图函数
def plot_results(series, test, predictions, future_dates=None, future_predictions=None, title="", name = None):
    plt.figure(figsize=(10, 6))
    plt.plot(series.index, series[need_to_predict].values, color='blue', label='原始数据')
    
    if predictions is not None:
        plt.plot(series.index[-len(test):], predictions, color='red', linestyle='--', label='测试预测')
    
    if future_dates is not None and future_predictions is not None:
        plt.plot(future_dates, future_predictions, color='green', linestyle='--', label='未来预测')
    ...
    plt.tight_layout()
    plt.savefig(f"out_file/figs/{need_to_predict} - {name} 合同签订情况")
    # plt.show()
...
# 绘制测试预测结果
plot_results(series, test, test_predictions, title=f"{need_to_predict} - 测试", name = "测试")

# 生成未来时间序列
future_dates = pd.date_range(start=series.index[-1] + pd.DateOffset(years=1), periods=years_to_predicted, freq='YS')
plot_results(series, test, None, future_dates, future_predictions, title=f"{need_to_predict} - 未来{years_to_predicted}年", name = "预测")

dic = {
    "日期": future_dates,
    f"{need_to_predict}": future_predictions
}
dics = DataFrame(dic)
dics[f"{need_to_predict}"] = dics[f"{need_to_predict}"].astype(int)
dics["日期"] = pd.to_datetime(dics["日期"]).dt.strftime('%Y-%m-%d')
dics.to_excel(f'out_file/excels/{need_to_predict}的未来{years_to_predicted}年合同签订项数量预测.xlsx', index=False)
